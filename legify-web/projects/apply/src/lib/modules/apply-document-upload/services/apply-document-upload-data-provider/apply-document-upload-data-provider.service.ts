...
@Injectable()
export class ApplyDocumentUploadDataProviderService {
  protected readonly allDocumentsSubj: BehaviorSubject<SupportingDocument[]> = new BehaviorSubject([]);
  ...
  public uploadDocument(documentToUpload: SupportingDocument): Observable<SupportingDocument[]> {
    const uploadDocument$ = of(documentToUpload);

    return uploadDocument$.pipe(concatMap((uploadedDocument) => this.addNewDocument(uploadedDocument)));
  }
  ...
  public deleteDocument(
    ...
    return deleteDocument$.pipe(concatMap(() => this.removeDocumentAtIndex(indexOfDocumentToRemove)));
  }
  ...
  public replaceDocument(
    ...
    return replaceDocument$.pipe(
      concatMap((uploadedDocumentReplacement) =>
        this.replaceDocumentAtIndex(indexOfDocumentToReplace, uploadedDocumentReplacement)
      )
    );
  }

  public addNewDocument(newDocument: SupportingDocument): Observable<SupportingDocument[]> {
  ...
  public removeDocumentAtIndex(indexOfDocumentToRemove: number): Observable<SupportingDocument[]> {
    return this.allDocuments$.pipe(
      map((allDocuments) => {
        const updatedDocuments = [...allDocuments];
  ...
  public replaceDocumentAtIndex(
    ...
    return this.allDocuments$.pipe(
      map((allDocuments) => {
        const updatedDocuments = [...allDocuments];
        updatedDocuments.splice(indexOfDocumentToReplace, 1, documentReplacement);
